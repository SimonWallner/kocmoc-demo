...
void util::safeDeleteArray(void* p)
		{
			if (p != NULL)
			{
				delete[] p;
...
// generators
RenderMesh* util::generator::generateStars()
{
...
LineGizmo* util::generator::generateOriginGizmo()
{
...
RenderMesh* util::generator::generateTriangle()
	{
...
// geometry
bool util::geometry::inside(const double d, const dvec3& n, const dvec3& point)
{
	double distance = glm::dot(point, n) - d;
	return (distance < 0);
}

double util::geometry::intersect(const double d, const dvec3& n, const dvec3& p1, const dvec3& p2)
{
